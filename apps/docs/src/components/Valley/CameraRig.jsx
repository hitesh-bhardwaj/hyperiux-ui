...
export default function CameraRig({ curve, cameraProps, scrollConfig = {} }) {
  ...
  // No scroll-driven movement: camera progress is time-driven only.
  ...
  useFrame((state, delta) => {
    ...
    // Smooth lerp toward scroll target (handles wrapping)
    let diff = scroll.current - progress.current;
    if (diff > 0.5) diff -= 1;
    if (diff < -0.5) diff += 1;
    progress.current = ((progress.current + diff * (scrollConfig.lerpSpeed ?? 0.05)) % 1 + 1) % 1;
    const p = curve.getPointAt(progress.current);
    const t = curve.getTangentAt(progress.current);

    container.current.position.copy(p);

    // Compute new quaternion for the container group to smoothly orient along the path
    // The target "look-at" direction is -t (i.e., looking down the curve, not up)
    ...
    if (!intro.current.done) {
      intro.current.angle +=
        (0 - intro.current.angle) * Math.min(1, 2 * delta);
      breath.current.rotation.x = intro.current.angle;
    ...
    camera.rotation.y +=
      (-maxRX * animMouse.current.x - camera.rotation.y) * 0.1;
    camera.rotation.x +=
      (-maxRY * animMouse.current.y - camera.rotation.x) * 0.1;
  });
