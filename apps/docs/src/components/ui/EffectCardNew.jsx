...
export function EffectCard({ effect, priority = false }) {
  ...
  return (
    <motion.div
      ...
      className="group relative bg-[#555555]/33 p-5 pb-[0.01vw] rounded-[1.5vw] border-border/50 overflow-hidden hover:shadow-2xl border hover:border-primary/50 backdrop-blur-md"
      onMouseEnter={handleMouseEnter}
      onMouseLeave={() => setIsHovered(false)}
    >
      {/* Clickable Preview Area */}
      <Link href={`/effects/${effect.name}`} className="block">
        <div className="aspect-video bg-black/20 rounded-[1vw] overflow-hidden relative">
          {/* Static Image */}
          <Image
      ...
      <div className="flex items-center justify-between py-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {(effect.categories?.length ? effect.categories : [effect.category]).map((cat) => (
            <span
              key={cat}
              className="px-2.5 py-0.5 bg-white border border-border/50 backdrop-blur-sm text-sm font-medium font-sans text-[#3C3C3C] capitalize "
