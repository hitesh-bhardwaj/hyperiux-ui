...
/**
 * Slot layout (relative to center card):
 *   0  = center  — front, full size
 *   1  = right-near  — 1st card stacked behind, peeking right
 *   2  = right-far   — 2nd card stacked behind, peeking further right
 *  -1  = left-near
 *  -2  = left-far
 *  null = fully hidden (snapped instantly, not animated)
 */
...
export default function CardSlider({ cards }) {
  ...
  useEffect(() => {
    ...
    cards.forEach((cardEl, i) => {
      ...
      handlers.push({ cardEl, handleMove, handleLeave });
    });

    return () => {
      handlers.forEach(({ cardEl, handleMove, handleLeave }) => {
  ...
  return (
    <section className="flex flex-col items-center px-6 pt-20 pb-12 select-none">
      ...
      <h3 className="text-5xl font-medium text-center leading-none tracking-tight text-gray-900 dark:text-white">
        Get your license.
      </h3>
      ...
      <div className="max-sm:grid max-sm:grid-cols-2 max-sm:place-items-center max-sm:gap-4 flex items-center justify-center">
        <button
          ...
          className="max-sm:hidden  w-9 h-9 rounded-full border border-gray-200 dark:border-gray-700 flex items-center justify-center text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors shrink-0 relative z-30"
