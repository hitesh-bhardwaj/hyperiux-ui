...
export function CurvedPlaneV2({ images = defaultImages }) {
  ...
  // Each column gets the same images array; triplicate for 3 columns
  ...
  useEffect(() => {
    ...
    const updateMeshes = () => {
      ...
      meshes.forEach((mesh, i) => {
        ...
        if (tablet && col === 1) { mesh.visible = false; return; }
        mesh.visible = true;

        const colImages = columnImages[col];
        const scrollPosition = offset + row;
        const imgIndex = Math.floor(scrollPosition) % colImages.length;
        const normalizedIndex = imgIndex < 0 ? imgIndex + colImages.length : imgIndex;
        const globalTextureIndex = images.length * col + normalizedIndex;
        ...
        let xPos;
        if (tablet) {
          xPos = col === 0 ? -SPACING_X / 2 : SPACING_X / 2;
        } else {
        ...
        mesh.material.uniforms.uOffset.value.set(0, deform * DEFORMATION_INTENSITY * deformDirection);
      });
    };
    ...
    const onWheel = (e) => {
      ...
      velocityRef.current = Math.max(-MAX_SCROLL_VELOCITY, Math.min(MAX_SCROLL_VELOCITY, delta * 0.4));
      const deformIntensity = Math.max(-MAX_DEFORMATION, Math.min(MAX_DEFORMATION, delta * DEFORMATION_SENSITIVITY));
      targetDeformationRef.current = deformIntensity;
  ...
  useEffect(() => {
    const getSwiperBounds = () => {
      ...
      const numCols = tablet ? 2 : 3;
      const totalWidth = numCols * FIXED_IMAGE_WIDTH + (numCols - 1) * 150;
      return { left: vw / 2 - totalWidth / 2, right: vw / 2 + totalWidth / 2 };
    };
    ...
    const handleMouseMove = (e) => {
      ...
      gsap.to(followerRef.current, { x: e.clientX - 50, y: e.clientY - 50, duration: 0.35, ease: "power2.out", overwrite: "auto" });
      ...
      if (isOverSwiper && !isFollowerVisible.current) {
        ...
        gsap.to(followerRef.current, { opacity: 0, scale: 0, duration: 0.3, ease: "power2.out" });
      }
    };
